...
class WeeblrampModel_Renderer extends WeeblrampClass_Model {
	...
	protected function loadData() {
		...
		if ( WeeblrampConfig_Customize::SOCIAL_BUTTONS_LOCATION_NONE != $this->customizeConfig->get( 'social_buttons_location' ) ) {
			$data['social_buttons']      = $this->getElementData( 'socialbuttons', $data, array(
				'types' => array(),
				'theme' => 'colors',
				'style' => 'rounded'
			) );
		...
		if ( WeeblrampConfig_Customize::LINK_TO_SITE_NOTIFICATION == $this->customizeConfig->get( 'show_link_to_main_site' ) && $this->customizeConfig->isTruthy( 'link_to_main_site_text' ) ) {
			...
			$data['link_to_main_site'] = array(
				'text'         => $this->customizeConfig->get( 'link_to_main_site_text' )
				                  . '<a href="' . $data['canonical'] . '"><button>' . $this->customizeConfig->get( 'link_to_main_site_link_text' ) . '</button></a>'
				,
		...
		if ( $this->router->isErrorPage() ) {
			$data['metadata']['title']       = __( 'The page you requested was not found on our site!' );
			$data['metadata']['description'] = __( 'Page not found' );
			$data['metadata']['keywords']    = '';
			$data['error_title']             = __( 'Oops! that page cannot be found.' );
			$data['error_body']              = __( 'We do not have any page at the address you requested. Please use the menu at the top to navigate to other pages of the site.' );
			$data['error_footer']            = sprintf( __( 'You requested %s' ), WblWordpress_Helper::getCurrentRequestUrl() );
	...
	private function autop( $text ) {
		 ...
		 * @return string
		 */
		return apply_filters( 'weeblramp_wpautop', wpautop( $text ) );
	}
	...
	private function getJsonldData( $data ) {
		...
		try {
			...
			$imageSize['width']       = 0;
			$imageSize['height']      = 0;
			$publisherImageDescriptor = $this->buildValidJsonLdImageDescriptor( $publisherImageUrl, $imageSize, $minWidth = 0, $minHeight = 0 );
	...
	private function sanitizeJsonLd( $jsonld, $pageData ) {
		...
		if ( empty( $jsonld['description'] ) ) {
			...
			if ( empty( $jsonld['description'] ) ) {
				// use meta data
				$jsonld['description'] = wbArrayGet( $pageData, array( 'metadata', 'decription' ), '' );
			}
	...
	private function findPageImage( $data ) {
		...
		if ( ! empty( $data['user_set_data'] ) && ! empty( $data['user_set_data']['image'] ) && ! empty( $data['user_set_data']['image']['url'] ) ) {
			...
			$image               = $this->buildValidJsonLdImageDescriptor( $data['user_set_data']['image']['url'], $imageSize );
		}
		...
		if ( WeeblrampHelper_Version::isFullEdition() && empty( $image ) ) {
			$fallbackImage = $this->userConfig->get( 'pages_fallback_image' );
			if ( ! empty( $fallbackImage ) ) {
				$imageSize = array( 'width' => 0, 'height' => 0 );
				$image     = $this->buildValidJsonLdImageDescriptor( $fallbackImage, $imageSize );
			}
		}
		...
		foreach ( $content as $contentItem ) {
			if ( ! empty( $contentItem['featured_image'] ) ) {
				...
				if ( ! empty( $imageUrl ) ) {
					...
					$imageSize = wbArrayGet( $contentItem['featured_image'], 'image_meta', array(
						'width'  => 0,
						'height' => 0
					) );
	...
	private function _extractImageFromContent( $match ) {
		...
		if ( ! empty( $match[1] ) && empty( $this->_extractedImage ) ) {
			...
			$this->_extractedImage = $this->buildValidJsonLdImageDescriptor( $attributes['src'], $imageSize );
		}
	 ...
	 * json+ld manifest
	 * By default, test image dimensions against configured AMP min width and height
	 * Pass 0 values for minWidth and minHeight to disable size test
	 *
	...
	private function buildValidJsonLdImageDescriptor(
		...
		if ( ! empty( $imageUrl ) ) {
			...
			$imageSize = WeeblrampHelper_Media::findImageSizeIfMissing( $imageUrl, $imageSize );
			$minWidth  = is_null( $minWidth ) ? $this->ampConfig->get( 'pageImageMinWidth' ) : 0;
			$minHeight = is_null( $minWidth ) ? $this->ampConfig->get( 'pageImageMinHeight' ) : 0;
			if ( ! empty( $imageSize['width'] ) && $imageSize['width'] >= $minWidth && ! empty( $imageSize['height'] ) && $imageSize['height'] >= $minHeight ) {
