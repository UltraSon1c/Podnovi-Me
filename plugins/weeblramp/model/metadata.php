...
class WeeblrampModel_Metadata extends WeeblrampClass_Model {
	...
	private function loadMeta() {
		...
		switch ( $class ) {
			case 'WP_Post':
				$this->__data['description'] = strip_tags( get_the_excerpt() );
				break;
			case 'WP_Term':
	...
	private function loadOgp() {
		...
		if ( is_ssl() ) {
			$this->__data['ogp']['image_secure_url'] = $this->__data['ogp']['url'];
		}
	...
	/**
	 * Loads Google publisher ID from configuration
	 *
	 * @return $this
	 */
	private function loadPublisherId() {

		$this->__data['publisher_id'] = $this->userConfig->get( 'publisher_id' );
