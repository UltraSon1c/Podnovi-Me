...
class WblWordpress_Helper {
	...
	public static function getUploadsInfo( $subDir = 'weeblramp' ) {
		...
		if ( is_null( $uploadInfo ) ) {
			...
			$uploadInfo['path'] = wbSlashJoin( $baseWpUploadsDir, $subDir );
			if ( wp_mkdir_p( $uploadInfo['path'] ) ) {
				$uploadInfo['url'] = wbSlashJoin( $baseWpUploadsUrl, $subDir );
			} else {
				// default to uploads, so that we always have a dir
				$uploadInfo['path'] = $baseWpUploadsDir;
				$uploadInfo['url']  = $baseWpUploadsUrl;
			}
		}
