...
class WeeblrampHelper_Boot {
	...
	private static function installPluginManager() {
		...
		if ( ! $createdOrExists ) {
			$errors[] = sprintf( __( 'Unable to create directory for weeblrAMP plugins handler. We will not be able to disable plugins on AMP pages. (path: %s)' ), $muFolder );
		...
		if ( ! $copied ) {
			$errors[] = sprintf( __( 'Unable to copy weeblrAMP plugins handler to Must-use directory. We will not be able to disable plugins on AMP pages. (path: %s)' ), $muFolder );
	...
	private static function removePluginManager() {
		...
		if ( file_exists( $pluginFile ) ) {
			...
			if ( ! $deleted ) {
				$errors[] = sprintf( __( 'weeblrAMP was deactivated, but we could not to remove the weeblrAMP plugin manager from the Must-use directory. Please remove it manually (path: %s)' ), $pluginFile );
			}
		}
	...
	private static function isDevVersion() {
		...
		return strpos( '1.4.0', '_version_' ) !== false;
	}
