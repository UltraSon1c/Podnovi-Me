	...
	public static function checkSystem() {
		...
		try {
			...
			if ( ! self::$versionInfo->isCompatibleWith( 'php', PHP_VERSION ) ) {
				...
				$maxPhpString = empty( $maxPhp ) ? '' : sprintf( __( ', but less than %s' ), $maxPhp );
				$errors[]     =
					sprintf(
						__( 'weeblrAMP needs the PHP version running on your server to be %s or higher%s. You are using PHP version %s, which is not compatible. Please make sure you use a compatible version.' ),
			...
			if ( ! self::$versionInfo->isCompatibleWith( 'wp', $wp_version ) ) {
				...
				$errors[]    =
					sprintf(
						__( 'weeblrAMP needs your WordPress version to be %s or higher%s. You are using WordPress version %s, which is not compatible. Please make sure you use a compatible version.' ),
	...
	public static function getUpgradeSettingDefContent() {
		...
		$content = array(
			...
			'default'  =>
				...
				. wbJoin(
					'</li><li>',
					__( '<strong>Automatic forms conversion</strong>: Contact Form 7 and Gravity Forms', 'weeblramp' ),
					...
					__( 'Google Tag manager support', 'weeblramp' ),
