...
class WeeblrampConfig_User extends WblSystem_Config {
	...
	public function __construct( $options = array() ) {
		...
		add_filter(
			static::STORAGE_PREFIX . '_amp_post_types_user_selectable_taxonomies',
			array(
				$this,
				'filter_amp_post_types_user_selectable_taxonomies'
			)
		);
	...
	public function ajaxHandler() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( 'Unable to perform action, not authorized.' ) );
		} else {
			try {
	...
	/**
	 * Override to get the built-in default list of user selectable
	 * taxonomies.
	 *
	 * @return mixed
	 */
	public function filter_amp_post_types_user_selectable_taxonomies( $taxonomies ) {

		$taxonomies = array_merge(
			$taxonomies,
			WeeblrampFactory::getThe( 'weeblramp.config.system' )
			...
			case self::OPTION_DISQUS_SHORTNAME:
				...
				if ( ! empty( $showIf ) ) {
					$__data['show-if-attrs'] = array(
						'data-show_if_id'   => WblSystem_Strings::asHtmlId(
	...
	private function sanitizeAmpSuffix() {
		...
		if ( empty( $this->beingSanitizedData['amp_suffix'] ) ) {
			...
			add_settings_error( 'weeblramp-settings', 'weeblramp_invalid_amp_suffix', __( 'AMP suffix cannot be empty. It was restored to its previous value.' ) );
		} else {
	 ...
	 * @return string
	 */
	private function getSavingPageSelectionHash( $data ) {
