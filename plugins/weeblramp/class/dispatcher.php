...
class WeeblrampClass_Dispatcher extends WeeblrampClass_Base {
	...
	public function dispatch() {
		...
		add_action( 'old_slug_redirect_url', array( $this, 'filterOldSlugRedirect' ) );
	...
	private function checkDebugAccess() {
		...
		if ( WeeblrampHelper_Route::shouldBlockAmpRequest() ) {
			// trigger a 403
			WblSystem_Http::abort( WblSystem_Http::RETURN_FORBIDDEN, __( 'weeblrAMP: missing debug token in URL.' ) );
		}
	}
	...
	private function renderRegularHtmlPage() {
		...
		add_filter(
			...
			// make sure we process rawest possible content,
			// we specifically want to remove our tags prior
			// to wpautop being run, or else the content will have
			// unwanted <br /> tags
			8
		);
	}
