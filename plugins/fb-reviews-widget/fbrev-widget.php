...
class Fb_Reviews_Widget extends WP_Widget {
    ...
    function fbrev_widget_scripts($hook) {
        if ($hook == 'widgets.php' || ($hook == 'customize.php' && defined('SITEORIGIN_PANELS_VERSION'))) {
            ...
            wp_register_style('fbrev_widget_css', plugins_url('/static/css/fbrev-widget.css', __FILE__));
            wp_enqueue_style('fbrev_widget_css', plugins_url('/static/css/fbrev-widget.css', __FILE__));
    ...
    function widget($args, $instance) {
        ...
        if (fbrev_enabled()) {
            ...
            echo $before_widget;
            $response = fbrev_api_rating($page_id, $page_access_token, $instance, 'widget', $cache, $api_ratings_limit);
            ...
            if (isset($response_json->data)) {
                ...
                ?>
                <div class="fbrev-error" style="padding:10px;color:#B94A48;background-color:#F2DEDE;border-color:#EED3D7;">
                    <?php echo fbrev_i('Facebook API Ratning: ') . $response_data; ?>
                </div>
                <?php
            }
            echo $after_widget;
