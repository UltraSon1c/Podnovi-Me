	...
	class Rm_Mailer_Hubspot_CRM{
		...
		function update_hubspot_authentication(){
			...
			if( is_array( $lists->lists ) && !empty( $lists->lists ) ) {
				...
				/*print_r(json_encode(array(
					'status' => "error",
					'message' => __( "You have zero static lists in your HubSpot account. You must have at least one static list before integration." , "icegram-rainmaker" )
				)));
				die();*/
			}
        	else
			...
			/*$hs_lists = array();
			$html = $query = '';
			$html .= '<select disabled id="hubspot-list" class="rm-list-select" name="form_data[hubspot_lists]">';
